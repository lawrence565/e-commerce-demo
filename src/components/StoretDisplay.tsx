...
function StoreDisplay(props: { type: string }) {
    ...
    return (
      <div className="p-2" key={index}>
        <Link to={`/stores/${product.category}/${product.id}`}>
          <Card 
            className="group hover:bg-white/80 transition-all duration-500 h-full flex flex-col p-3 border border-transparent hover:border-black/10 hover:shadow-lg"
            variant="glass"
          >
             <div className="aspect-[4/3] w-full overflow-hidden rounded-xl bg-[#efe7da]/50 relative">
                <LazyImage
             ...
             <div className="mt-4 flex flex-col justify-between flex-1 gap-1 px-1">
                <div className="flex justify-between items-start gap-2">
                    <h3 className="text-lg font-bold text-black leading-tight headline-serif group-hover:text-[color:var(--color-accent-deep)] transition-colors line-clamp-2">
                        {product.title}
                    </h3>
                </div>
                <div className="flex items-center justify-between mt-2 pt-3 border-t border-black/10">
                    ...
                    <span className="text-[color:var(--color-accent-deep)] font-bold text-lg font-mono">
                        ${product.price}
                    </span>
                </div>
             </div>
  ...
  return (
    <div className="h-fit">
      ...
      >
        {pages[pageIndex] ?? (
          <div className="col-span-2 lg:col-span-3 text-center py-12 text-black/50 headline-serif text-xl">
            目前沒有商品
          </div>
        )}
      </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center mt-12 mb-8 gap-4">
            <button
              ...
              className="p-2 rounded-full hover:bg-black/5 disabled:opacity-30 disabled:hover:bg-transparent transition-colors text-black"
            >
            ...
            <div className="flex gap-2">
              {Array.from({ length: totalPages }, (_, index) => (
                <button
                  ...
                  className={`w-10 h-10 rounded-full font-bold transition-all duration-300 headline-serif ${
                    pageIndex === index
